...
exports.commaAndString = (array) => {
  var string = "";
  if (array.length === 2) {
    string = array.join(",").replace(",", " and ");
  } else if (array.length > 2) {
    string = array.join(", ").replace(/, ([^,]*)$/, " and $1");
  }
  return string;
};
exports.commaOrString = (array) => {
  var string = "";
  if (array.length === 2) {
    string = array.join(",").replace(",", " or ");
  } else if (array.length > 2) {
    string = array.join(", ").replace(/, ([^,]*)$/, " or $1");
  }
  return string;
};
